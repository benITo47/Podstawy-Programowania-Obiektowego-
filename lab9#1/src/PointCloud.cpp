...
int Point2D::DISPLAY_PRESITION = 2;
...
{
    Point[0] = static_cast<double>(x);
    Point[1] = static_cast<double>(x);
}

std::string Point2D::to_string(const Point2D& refPoint)
{
    std::string RetString = "(";
    RetString += std::to_string(refPoint.Point[0]);
    RetString += ",";
    RetString += std::to_string(refPoint.Point[1]);
    RetString += ")";
    return RetString;
}
...
void VPointCloud2D::addPoint()
{
    Point2D* newOne = new Point2D(0);
